...
void translate(MatrixTypes aType, vec3& v)
{
	float mat[16];
...
void scale(MatrixTypes aType, vec3& v)
{
	float mat[16];
...
void rotate(MatrixTypes aType, float angle, vec3& v)
{
	float mat[16];
	float t[3];
	...
	//	mat[0] = x2 + (y2 + z2) * co; 
	mat[0] = co + x2 * (1 - co);// + (y2 + z2) * co; 
	mat[4] = t[0] * t[1] * (1 - co) - t[2] * si;
	mat[8] = t[0] * t[2] * (1 - co) + t[1] * si;
	mat[12] = 0.0f;

	mat[1] = t[0] * t[1] * (1 - co) + t[2] * si;
	//	mat[5] = y2 + (x2 + z2) * co;
	mat[5] = co + y2 * (1 - co);
	mat[9] = t[1] * t[2] * (1 - co) - t[0] * si;
	mat[13] = 0.0f;

	mat[2] = t[0] * t[2] * (1 - co) - t[1] * si;
	mat[6] = t[1] * t[2] * (1 - co) + t[0] * si;
	//	mat[10]= z2 + (x2 + y2) * co;
	mat[10] = co + z2 * (1 - co);
	mat[14] = 0.0f;
